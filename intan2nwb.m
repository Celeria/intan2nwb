...
for ii = to_proc
    ...
    try
        ...
        if send_slack_alerts
            SendSlackNotification( ...
                    SLACK_ID, ...
                    [nwb.identifier ': [' s2HMS(ttt) '] Error in recording sheet entry or folder name. Probably. Check that first, in any case, processing failed on this session.'], ...
                    'preprocess', ...
                    'iJakebot', ...
                    '', ...
                    ':robot_face:');
    ...
    for rd = 1 : num_recording_devices
        ...
        % Digital events
        [nwb,digital_failed] = i2nDIO(nwb, recdev{rd});
        ttt = toc;
        if send_slack_alerts
            if ~digital_failed
                SendSlackNotification( ...
                ...
                SendSlackNotification( ...
                    SLACK_ID, ...
                    [nwb.identifier ': [' s2HMS(ttt) '] dev-' num2str(rd-1) ', Digital I/O extraction not entirely successful.'], ...
                    'preprocess', ...
                    'iJakebot', ...
                    '', ...
                    ':robot_face:');
